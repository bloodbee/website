...
const ProjectsTemplate: FC<ProjectsTemplateProps> = ({ data, pageContext }) => {
...
export const Head: FC<ProjectsTemplateProps> = ({ pageContext }) => {
  const { title, description } = useSiteMetadata();
  const {
    pagination: { currentPage: page },
  } = pageContext;
  const pageTitle = page > 0 ? `Projects - Page ${page} - ${title}` : title;
